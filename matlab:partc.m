function puma560_computed_torque_dual_sim()
    % PUMA 560 Computed Torque Control: Compare Nominal vs 2kg Payload
    clear; clc; close all;
    ...
    %% Run Simulations
    [q_nom, err_nom, ee_nom] = run_simulation(nominal_robot, nominal_robot, q_des, qd_des, qdd_des, Kp, Kd, t, dt, 'Nominal_Robot');
    [q_payload, err_payload, ee_payload] = run_simulation(nominal_robot, actual_robot, q_des, qd_des, qdd_des, Kp, Kd, t, dt, 'With_2kg_Payload');
    ...
    %% 6. Plot End-Effector Paths
    figure('Name', 'End-Effector Path Comparison');
    plot3(ee_nom(:,1), ee_nom(:,2), ee_nom(:,3), 'b--', 'LineWidth', 2); hold on;
    plot3(ee_payload(:,1), ee_payload(:,2), ee_payload(:,3), 'r-', 'LineWidth', 2);
    xlabel('X [m]'); ylabel('Y [m]'); zlabel('Z [m]');
    title('End-Effector Path: Nominal vs 2kg Payload');
    legend('Nominal', 'Payload');
    grid on; axis equal;

    %% 7. Numerical Error Comparison
...
function [q_log, error_log, ee_log] = run_simulation(nominal_robot, actual_robot, q_des, qd_des, qdd_des, Kp, Kd, t, dt, label)
    N = length(t);
    q = q_des(1,:);
    qd = zeros(1,6);
    q_log = zeros(N,6);
    ...
    % Setup video writer
    video_filename = sprintf('%s_simulation.mp4', label);
    v = VideoWriter(video_filename, 'MPEG-4');
    v.FrameRate = round(1/dt);
    open(v);

    figure('Name', ['Simulation - ', label]);
    for i = 1:N
        q_ref = q_des(i,:);
        qd_ref = qd_des(i,:);
        qdd_ref = qdd_des(i,:);
        ...
        tau = M_nom * (qdd_ref' + Kd*ed' + Kp*e') + C_nom*qd' + G_nom;
        ...
        q_log(i,:) = q;
        error_log(i,:) = e;
        ee_log(i,:) = transl(actual_robot.fkine(q));

        actual_robot.plot(q, 'delay', 0, 'noshadow', 'nowrist');
        title(sprintf('%s - Time: %.2f s', label, t(i)));
        drawnow limitrate;
